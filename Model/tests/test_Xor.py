...
def test_xor_logic_state_and_change(a, b, expected):
    ...
    assert xor_gate.state["outValue"] == expected, f"Xor.state['outValue'] should be {expected} after eval() with inputs {a}, {b}."
...
def test_xor_state_changes_multiple_times():
    ...
    changed = xor_gate.eval()
    assert xor_gate.state["outValue"] is False, "Xor.state['outValue'] should be False after both inputs are False."
    ...
    assert xor_gate.state["outValue"] is True, "Xor.state['outValue'] should be True after one input is True."
    ...
    assert xor_gate.state["outValue"] is False, "Xor.state['outValue'] should be False after both inputs are True."
