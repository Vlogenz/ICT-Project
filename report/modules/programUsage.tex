...
The level selection screen presents available levels in an organized grid format where levels are grouped into thematic blocks such as basic gates, combinational circuits, and sequential logic. Graphical lines connect related levels to clearly show the learning progression, while status indicators display completion status with "Done" markers or "Locked" labels for unavailable content. A recommendation system highlights suggested next steps in the learning path to guide students through the most effective sequence of exercises.
...
The application provides automated solution verification through a check solution button that gives immediate feedback on circuit correctness. Comprehensive testing validates circuit behavior using multiple test cases, while some levels integrate output prediction exercises that require students to predict results before simulation. Clear success and failure messages provide specific guidance for improvements, and successful completion automatically updates the user's progress in the system.
...
For entering the environment, Linux users should execute \texttt{source env/bin/activate}, while Windows users need to run \texttt{\& "./env/Scripts/Activate.ps1"}. If Windows displays an error about running scripts being disabled, first execute \texttt{Set-ExecutionPolicy -ExecutionPolicy Unrestricted -Scope Process}.

After activating the environment, install the required packages using \texttt{pip install -r requirements.txt}.

Finally, run the application with \texttt{python3 -m src.main}.
